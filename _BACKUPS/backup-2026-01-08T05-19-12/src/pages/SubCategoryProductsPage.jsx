...
const SubCategoryProductsPage = () => {
  ...
  const [newDescription, setNewDescription] = useState(''); // Courte
  const [newLongDescription, setNewLongDescription] = useState(''); // Longue (Nouveau)
  ...
  const handleSave = async (e) => {
    ...
    const payload = { 
      ...
      description: newDescription, // Courte
      long_description: newLongDescription, // Longue
  ...
  const openEditModal = (prod) => {
    ...
    setNewLongDescription(prod.long_description || ''); // Charger la description longue
    setNewImage(null);
    setShowModal(true);
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 font-sans min-h-screen">
        ...
        {products.map((prod) => (
          <div key={prod.id} className="group relative bg-white rounded-3xl border border-slate-100 shadow-lg hover:shadow-2xl transition-all overflow-hidden flex flex-col">
            ...
            <div className="p-5 flex flex-col flex-grow bg-slate-50 border-t border-slate-100">
              ...
              <p className="text-sm text-slate-500 font-medium line-clamp-2">{prod.description || "Pas de description courte"}</p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 relative animate-in zoom-in max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              {/* DESCRIPTION COURTE */}
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Description Courte (Visible sur la carte)</label>
                <textarea rows="2" value={newDescription} onChange={e => setNewDescription(e.target.value)} className="w-full border border-slate-200 p-3 rounded-xl bg-slate-50 text-sm outline-none focus:ring-2 focus:ring-blue-500" placeholder="Bref résumé..."></textarea>
              </div>

              {/* DESCRIPTION LONGUE (NOUVEAU) */}
              <div>
                <label className="block text-xs font-bold text-blue-600 uppercase mb-1">Description Détaillée (Visible sur la page produit)</label>
                <textarea rows="6" value={newLongDescription} onChange={e => setNewLongDescription(e.target.value)} className="w-full border border-blue-200 p-3 rounded-xl bg-blue-50/50 text-sm outline-none focus:ring-2 focus:ring-blue-500" placeholder="Description complète, caractéristiques techniques, dimensions..."></textarea>
              </div>
