    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          role: editForm.role,
          phone: editForm.phone,   // Ajout du téléphone
          siret: editForm.siret    // Ajout du SIRET
        })
        .eq('id', id);
  ...
  const handleValidateUser = async (user) => {
    ...
    try {
      ...
      if (!n8nResponse.ok) {
        ...
        throw new Error(`n8n/Iabako: ${errorMsg || 'Forbidden (Vérifiez le Token)'}`);
      }
      ...
      alert("Succès ! Client créé dans Iabako avec ses nouvelles infos.");
      fetchUsers();
    } catch (err) {
      alert("ERREUR : " + err.message);
    } finally {
              ...
              <tr key={user.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="p-6">
                  {editingId === user.id ? (
                    <div className="flex flex-col gap-2">
                      ...
                      />
                      <select 
                        className="bg-white p-2 rounded border font-bold text-xs mt-1"
                        value={editForm.role}
                  ...
                  {editingId === user.id ? (
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <input 
                          className="bg-white p-2 rounded border text-xs flex-1"
                ...
                <td className="p-6 text-right">
                  <div className="flex gap-2 justify-end">
                    {editingId === user.id ? (
                      <>
                        <button onClick={() => saveChanges(user.id)} className="p-3 bg-green-500 text-white rounded-xl hover:bg-green-600 shadow-lg transition-all"><Save size={18}/></button>
                      ...
                      <>
                        <button onClick={() => startEdit(user)} className="p-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Modifier"><Edit2 size={18}/></button>
                        {!user.is_verified && (
                          <button 
