...
import { Plus, X, Loader, Pencil, ArrowLeft, ShoppingCart, Package } from 'lucide-react';

const ProductsPage = () => {
  const { subcategoryId } = useParams(); // On récupère l'ID de la sous-catégorie
  ...
  const fetchData = async () => {
    ...
    // 1. Info de la Sous-Catégorie parente
    const { data: subData } = await supabase
      .from('subcategories')
      .select('name, category_id')
      .eq('id', subcategoryId)
      .single();
    setParentSub(subData);
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-12 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Navigation Retour */}
        <Link 
          to={parentSub ? `/categories/${parentSub.category_id}` : '/categories'} 
          className="inline-flex items-center gap-2 text-slate-500 hover:text-blue-600 mb-8 transition"
        >
          <ArrowLeft size={20} /> Retour aux sous-catégories
        </Link>

        {/* Titre */}
        <div className="mb-12">
          <h1 className="text-3xl font-extrabold text-slate-900">
            {parentSub ? parentSub.name : 'Chargement...'}
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          {products.map((prod) => (
            <div key={prod.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-lg transition group relative flex flex-col">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <p className="text-slate-500 text-sm mb-4 line-clamp-2 flex-grow">{prod.description}</p>
                
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100">
                  <span className="text-xl font-bold text-blue-600">
                    {prod.price ? `${prod.price} €` : 'Sur devis'}
                  </span>
                  <div className={`text-xs font-bold px-2 py-1 rounded-full ${prod.stock > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8 relative animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <textarea name="description" rows="3" value={formData.description} onChange={handleChange} className="w-full border border-slate-300 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Caractéristiques techniques..." />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">Prix (€)</label>
                  <input name="price" type="number" step="0.01" value={formData.price} onChange={handleChange} className="w-full border border-slate-300 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: 12.50" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">Stock</label>
                  <input name="stock" type="number" value={formData.stock} onChange={handleChange} className="w-full border border-slate-300 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: 100" />
                </div>
              </div>
