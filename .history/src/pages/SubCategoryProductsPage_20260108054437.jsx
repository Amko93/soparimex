...
import { Plus, X, Loader, ArrowLeft, Pencil, Trash2, AlertTriangle, Package } from 'lucide-react';

const SubCategoryProductsPage = () => {
  const { id } = useParams(); // ID de la sous-catégorie
  ...
  // États Formulaire (Ajout/Modif Produit)
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // 1. Infos de la sous-catégorie (pour l'en-tête)
  ...
  const handleSave = async (e) => {
    ...
    const payload = { 
      ...
      subcategory_id: id // On lie à la sous-catégorie
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 font-sans min-h-screen">
      {/* Bouton Retour (Dynamique vers la catégorie parente si possible, sinon vers la liste) */}
      ...
      {/* --- EN-TÊTE BLANC (Style Image 3e3ca5) --- */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        {products.map((prod) => (
          <div key={prod.id} className="group relative bg-white rounded-3xl border border-slate-100 shadow-lg hover:shadow-2xl transition-all overflow-hidden flex flex-col">
            ...
            <div className="p-5 flex flex-col flex-grow bg-slate-50 border-t border-slate-100">
              ...
              {/* Bouton "Voir" ou "Commander" (Décoratif pour l'instant) */}
              <button className="mt-4 w-full py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-colors">
                VOIR LE PRODUIT
              </button>
            </div>

            {/* Outils Admin (Modif/Suppr) */}
            {session && (
              <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                <button 
                  onClick={() => { setEditingProduct(prod); setNewName(prod.name); setNewDescription(prod.description); setShowModal(true); }} 
                  className="bg-white p-2 rounded-lg text-slate-600 hover:text-blue-600 shadow-md border border-slate-100"
                >
                  <Pencil size={14} />
                </button>
                <button 
                  onClick={() => { setItemToDelete(prod.id); setShowDeleteConfirm(true); }} 
                  className="bg-white p-2 rounded-lg text-red-500 hover:bg-red-50 shadow-md border border-slate-100"
                >
                  <Trash2 size={14} />
                </button>
              </div>
        ...
        {/* CARTE D'AJOUT (Visible si admin) */}
        {session && (
          <div 
            onClick={() => { setEditingProduct(null); setNewName(''); setNewDescription(''); setNewImage(null); setShowModal(true); }} 
            className="min-h-[300px] border-3 border-dashed border-blue-200 bg-blue-50/30 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:bg-blue-50 hover:border-blue-500 transition-all text-blue-500 group gap-3"
          >
            <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform text-blue-600">
      ...
      {/* Message Vide (Si pas de produits et pas admin) */}
      {products.length === 0 && !session && (
         <div className="text-center py-20 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 mt-8">
           <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm text-slate-300">
             <Package size={32} />
           </div>
           <h3 className="text-xl font-bold text-slate-700 mb-2">Aucun produit pour le moment</h3>
           <p className="text-slate-400">Les produits de cette gamme seront ajoutés prochainement.</p>
         </div>
      )}
