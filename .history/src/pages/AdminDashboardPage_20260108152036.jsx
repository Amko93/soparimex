...
const AdminDashboardPage = () => {
  const [stats, setStats] = useState({
    ...
    subcategories: 0,
    products: 0,
    messages: 0 // On remplace pendingUsers par messages
  });
  ...
  const fetchStats = async () => {
    ...
    const [cat, sub, prod, msgs] = await Promise.all([
      supabase.from('categories').select('*', { count: 'exact', head: true }),
      supabase.from('subcategories').select('*', { count: 'exact', head: true }),
      supabase.from('products').select('*', { count: 'exact', head: true }),
      supabase.from('messages').select('*', { count: 'exact', head: true }) // On compte les messages
    ]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-slate-500 mt-2 font-medium">Bienvenue, Amir. Voici l'état de ton catalogue et de ta messagerie.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <Link to="/admin/messages" className="group bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center justify-between relative overflow-hidden">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <h3 className="text-2xl font-black text-slate-800">Messagerie</h3>
                <p className="text-slate-500 mt-1 font-medium">Lire et répondre aux demandes clients</p>
              </div>
            </div>
            <div className="w-12 h-12 rounded-full border border-slate-100 flex items-center justify-center group-hover:bg-purple-600 group-hover:text-white transition-colors">
              <ChevronRight size={24} />
            </div>
          </Link>
