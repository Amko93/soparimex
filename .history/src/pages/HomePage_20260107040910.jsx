    ...
    <div style={{ backgroundColor: 'var(--site-bg)' }} className="min-h-screen transition-colors duration-300">
      ...
      <section style={{ backgroundColor: 'var(--footer-bg)' }} className="py-24 px-8 text-center text-white">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link to="/categories" style={{ backgroundColor: 'var(--primary-button)' }} className="px-10 py-5 rounded-2xl font-black shadow-2xl flex items-center gap-2 hover:scale-105 transition-all text-white">
              {texts.btnHero} <ArrowRight size={20} />
            </Link>
            <Link to="/catalogues" className="px-10 py-5 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl font-black flex items-center gap-2 hover:bg-white/20 transition-all text-white">
      ...
      <section className="py-20 px-8 -mt-16 relative z-10">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
          {services.map((s, i) => (
            <div key={i} className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100 group transition-all">
      ...
      {/* Zone Expertise avec son fond propre */}
      <div style={{ backgroundColor: 'var(--expertise-bg)' }} className="py-24 transition-colors">
        <section className="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <div className="text-left">
            ...
            <p style={{ color: 'var(--main-text)' }} className="opacity-70 leading-relaxed text-lg mb-8">{texts.expertiseText}</p>
            <Link to="/contact" className="px-8 py-4 bg-white rounded-2xl font-black text-sm shadow-md border border-slate-100 hover:shadow-lg transition-all text-slate-800">
              En savoir plus
            </Link>
          </div>
