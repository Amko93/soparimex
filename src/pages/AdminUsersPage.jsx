...
import { Link } from 'react-router-dom'; // Import nécessaire pour le lien retour
// Vérifie le chemin "../supabaseClient" ou "../../supabaseClient" selon ton dossier
import { supabase } from '../supabaseClient'; 
import { 
  Search, Loader, CheckCircle2, Clock, 
  UserCheck, Save, Edit2, X, Phone, Fingerprint, Ban, ArrowLeft 
} from 'lucide-react';
...
const AdminUsersPage = () => {
  ...
  const handleToggleValidation = async (user) => {
    ...
    const newStatus = !user.is_validated; 
  ...
  const filteredUsers = users.filter(user => {
    ...
    return (
      ...
      (user.societe || "").toLowerCase().includes(search) ||
      (user.phone || "").toLowerCase().includes(search) || // Recherche par téléphone
      (user.siret || "").toLowerCase().includes(search)    // Recherche par SIRET
    );
  });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto font-sans bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col gap-6 mb-10">
        <Link to="/admin" className="inline-flex items-center gap-2 text-slate-500 hover:text-blue-600 font-bold transition w-fit">
          <ArrowLeft size={20} /> Retour au Dashboard
        </Link>

        <div className="flex flex-col md:flex-row justify-between items-end gap-4">
          <div>
            <h1 className="text-4xl font-black italic tracking-tighter uppercase text-slate-900">Gestion Utilisateurs</h1>
          ...
          <div className="relative w-full md:w-auto">
            ...
            <input 
              ...
              className="bg-white p-4 pl-12 rounded-2xl font-bold border-none w-full md:w-80 shadow-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all"
