...
const SearchResultsPage = () => {
  ...
  const query = searchParams.get('q'); // Récupère le mot tapé
    ...
    const performSearch = async () => {
      try {
        const { data, error } = await supabase
          .from('products')
          .select('*')
          .or(`name.ilike.%${trimmed}%,product_code.ilike.%${trimmed}%`);
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 font-sans min-h-screen">
      ...
      {isEmpty ? (
        <div className="text-center py-20 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200">
          ...
          <h3 className="text-xl font-bold text-slate-700">Veuillez entrer une recherche</h3>
          <p className="text-slate-400 mt-2">Utilisez la barre de recherche ci-dessus pour trouver un produit par nom ou référence.</p>
        </div>
      ) : loading ? (
            ...
            <div key={prod.id} className="group relative bg-white rounded-3xl border border-slate-100 shadow-lg hover:shadow-2xl transition-all overflow-hidden flex flex-col">
              ...
              <div className="p-5 flex flex-col flex-grow bg-slate-50 border-t border-slate-100">
                ...
                <div className="mt-4 flex flex-col gap-2">
                  <AddToSelectionButton product={prod} className="w-full py-2.5 text-xs" />
                  <Link 
                    to={`/product/${prod.id}`}
                    className="w-full py-3 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-colors block text-center"
                  >
                    VOIR LE PRODUIT
                  </Link>
                </div>
        ...
        <div className="text-center py-20 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200">
          ...
          <p className="text-slate-400">Essayez de chercher par nom ou par référence.</p>
        </div>
      )}
    </div>
  );
